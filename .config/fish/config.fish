...
# Apply Arabic keyboard layout on terminal startup
if status is-interactive
    hyprctl eval 'hl.config({ input = { kb_layout = "us,ara" } })' > /dev/null 2>&1
    hyprctl eval 'hl.config({ input = { kb_options = "grp:alt_shift_toggle" } })' > /dev/null 2>&1
end
