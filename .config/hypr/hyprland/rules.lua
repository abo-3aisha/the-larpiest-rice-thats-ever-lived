...
-- wofi menus (SUPER+S ctrl-center, SUPER+SPACE wifi list, wallpaper pickers)
hl.layer_rule({ match = { namespace = "wofi" }, animation = "popin 60%", blur = true })

-- swaync control center slides smoothly from the right (no blur — panel spans the whole screen; blur only on wofi) — notifications: NO blur
hl.layer_rule({ match = { namespace = "swaync-control-center" }, animation = "slide right 80%" })
hl.layer_rule({ match = { namespace = "swaync-notification-window" }, animation = "fade" })
